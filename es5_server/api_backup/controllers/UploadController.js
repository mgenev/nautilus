...
module.exports = {
    ...
    homepageTemplate: function homepageTemplate(req, res) {
    ...
    photo: function photo(req, res) {
        ...
        file.upload({
            ...
            _.series([function (callback) {
                // Resize thumb, preserve dimensions
                gm(uploadedFiles[0].fd).resize(300 + '>').gravity('Center').write(destDir + '/thumbs/' + fileName, function (error) {
                ...
                // Resize main
                gm(uploadedFiles[0].fd).resize(1024 + '>').gravity('Center').write(uploadedFiles[0].fd, function (error) {
                    if (error) res.send(500, error);
                    callback();
                });
            }, function (callback) {
...
resizePhoto = function (photo, path, width) {
    gm(photo).resize(width).write(root + '/public/img/uploads/' + name, function (err) {
